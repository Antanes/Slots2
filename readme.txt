...
В папке Models/Domain хранятся модель напитков Drinks и модель автомата(покупки) Machine. В модели Machine присутствует метод покупки Purchase, в котором содержится вся логика покупки напитка. В папке Models/ViewModel содержится модель представления. В файле ApplicationDbContext описано взаимодействие с базой данных.

В папке Controllers присутствует контроллер DrinkController, в котором содержаться все методы действий с базой данных и методы вспомогательной логики.
